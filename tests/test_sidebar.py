...
def test_sidebar_title_text(qapp):
    ...
    sb = Sidebar()
    title = sb.findChild(
        type(sb.findChildren(type(sb))[0].__class__)
        if sb.findChildren(type(sb))
        else object,
        "appTitle",
    )
...
def test_sidebar_fixed_width_200(qapp):
    from src.ui.components.sidebar import Sidebar

    sb = Sidebar()
    assert sb.maximumWidth() == 200
    assert sb.minimumWidth() == 200
...
def test_sidebar_section_headers_not_clickable(qapp):
    ...
    sb = Sidebar()
    sections = [
        w for w in sb.findChildren(QLabel) if w.objectName() == "sidebarSection"
    ]
